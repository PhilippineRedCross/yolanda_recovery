var width = $(window).width() - 15,
    height = 305;
var projection = d3.geo.mercator().scale(5000).center([124, 11]).translate([
    width / 2, height / 2
]);
fitProjection(projection, phlBoundingBox, [
    [0, 0],
    [width, height]
]);
var path = d3.geo.path().projection(projection);
var zoom = d3.behavior.zoom().translate([0, 0]).scale(1).scaleExtent([0, 17]).on(
    "zoom", zoomed);
...
function zoomed() {
    ...
    provinceGroup.attr("transform", "translate(" + d3.event.translate +
        ")scale(" + d3.event.scale + ")");
    municipGroup.attr("transform", "translate(" + d3.event.translate +
        ")scale(" + d3.event.scale + ")");
    brgyGroup.attr("transform", "translate(" + d3.event.translate +
        ")scale(" + d3.event.scale + ")");
}
var svg = d3.select("#map").append("svg").attr("width", width).attr("height",
    height);
...
function d3Start() {
    d3.json("data/admin1.json", function(data) {
        ...
        provinceGroup.selectAll("path").data(provinceData).enter().append(
                "path").attr("d", path).on("click", clickedProvince)
            .on("mouseover", function(d) {
                var tooltipText = "<strong>" + d.properties.name_1 +
                    "</strong>";
...
function loadSector(sector, target) {
    ...
    if (sector === "livelihood") {
        indicatorList = ["HH Support Beneficiaries Selected",
    ...
    if (sector === "shelter") {
        indicatorList = ["Shelter Repair Beneficiaries Selected",
            "First Distribution", "Second Distribution", "CGI",
    ...
    if (sector === "health") {
        indicatorList = ["Facility Selected",
            "Rehab / Construction Started", "Facility Equipped",
            "Construction & Handover Completed", "Community Selected",
            "# Households in Targeted Community",
            "Facilitators Trained", "CHVs Trained"
    ...
    if (sector === "education") {
        indicatorList = ["Classrooms Selected at School",
    ...
    if (sector === "watsan") {
        indicatorList = ["Household Latrines Construction Started",
    ...
    if (sector === "drr") {
        indicatorList = ["Community organized (RC 143)",
...
function calculateShelterOngoing(data) {
    $.each(data, function(index, item) {
        var ongoing = item["Construction Started"] - item[
            "Wooden Core Shelter Completed"] - item[
...
function calculateLatrineOngoing(data) {
        $.each(data, function(index, item) {
            var ongoing = item["Latrines Started (Core and Relocation)"] -
                item["Core Latrines Completed"] - item[
                    "Relocation Latrines Completed"];
            item["Shelter Latrine Construction Ongoing"] = ongoing;
            var schoolOngoing = item[
                "Schools w/ Latrine Construction Started"] - item[
                "Schools w/ Latrine Construction Completed"];
            item["Schools w/ Latrine Construction Ongoing"] =
                schoolOngoing;
        });
        return data;
    }
...
function parsePartners() {
    ...
    var partnerFilterHtml =
        '<button id="ALL-PARTNERS" class="btn btn-sm btn-donor filtering all" type="button" onclick="togglePartnerFilter(' +
        "'ALL-DONORS'" + ', this);"' +
    ...
    $.each(partnerList, function(index, partner) {
        var itemHtml = '<button id="' + partner +
            '" class="btn btn-sm btn-donor" type="button" onclick="togglePartnerFilter(' +
...
function changePartnerFilter() {
    ...
    $(currentSectorData()).each(function(index, record) {
        if (selectedPartner === record.Partner || selectedPartner ===
            "ALL-PARTNERS") {
...
function drawMunicipalities(d) {
    ...
    svg.transition().duration(750).call(zoom.translate(translate).scale(
        scale).event);
    var municipDisplay = municipGroup.selectAll("path").data(municipData.features
        .filter(function(d) {
            return d.properties.PCODE_PH1 === clickedPcode;
        }), function(d) {
            return d.properties.PCODE_PH2;
        });
    municipDisplay.enter().append("path").attr("d", path).on("click",
        clickedMunicip).on("mouseover", function(d) {
        var tooltipText = d.properties.name_2 + ", " + d.properties
            .name_1;
        $('#tooltip').append(tooltipText);
    }).on("mouseout", function() {
        $('#tooltip').empty();
...
function drawBarangays(d) {
    ...
    svg.transition().duration(750).call(zoom.translate(translate).scale(
        scale).event);
    var brgyDisplay = brgyGroup.selectAll("path").data(brgyData.features.filter(
        function(d) {
            return d.properties.PCODE_PH2 === clickedPcode;
        }), function(d) {
        return d.properties.PCODE_PH3;
    });
    brgyDisplay.enter().append("path").attr("d", path).on("mouseover",
        function(d) {
            var tooltipText = d.properties.name_3 + ", " + d.properties
                .name_2 + ", " + d.properties.name_1;
            $('#tooltip').append(tooltipText);
        }).on("mouseout", function() {
        $('#tooltip').empty();
    });
...
function createTable() {
    ...
    $(currentSectorData()).each(function(index, record) {
        if (selectedPartner === record.Partner || selectedPartner ===
            ...
            // operation overview
            if ("ALL" === activePcode) {
                if (breakdownRows.hasOwnProperty(record.Admin2) ===
                    false) {
                    breakdownRows[record.Admin2] = {
                ...
                $.each(indicatorList, function(index, indicator) {
                    if (isFinite(parseInt(record[indicator]))) {
                        totalRow[activePcode][indicator] +=
                            parseInt(record[indicator]);
                        breakdownRows[record.Admin2][
                            indicator
                        ] += parseInt(record[indicator]);
                    }
            ...
            if (record.Admin2 === activePcode) {
                if (breakdownRows.hasOwnProperty(record.Admin3) ===
                    false) {
                    breakdownRows[record.Admin3] = {
            ...
            if (record.Admin3 === activePcode) {
                if (breakdownRows.hasOwnProperty(record.Admin4) ===
                    false) {
                    breakdownRows[record.Admin4] = {
                        'name': record.Barangay
                    };
                    $.each(indicatorList, function(index, indicator) {
                        breakdownRows[record.Admin4][
                            indicator
    ...
    $.each(indicatorList, function(index, indicator) {
        columnCount++;
        $("#sector-accomplished_headers").append("<th>" + indicator +
            "</th>");
    });
    $("#sector-accomplished colgroup").html('<col style="width:20%">');
    var columnWidth = 80 / columnCount;
    for (i = 0; i < columnCount; i++) {
        $("#sector-accomplished colgroup").append('<col style="width:' +
            columnWidth.toString() + '%">');
    }
    for (x in totalRow) {
        var thisHtml = '<tr class="danger totalRow"><td>' + totalRow[x][
            'name'
        ] + "</td>";
        $.each(indicatorList, function(index, indicator) {
            thisHtml += "<td>" + formatCommas(totalRow[x][indicator]) +
                "</td>";
        });
        thisHtml += "</tr>";
    ...
    for (x in breakdownRows) {
        ...
        $.each(indicatorList, function(index, indicator) {
            thisHtml += "<td>" + formatCommas(breakdownRows[x][
                indicator
            ]) + "</td>";
        });
        thisHtml += "</tr>";
...
function colorMap() {
    ...
    for (entry in provinceList) {
        provinceGroup.selectAll("path").filter(function(d) {
            return d.properties.PCODE_PH1 === entry
        }).attr('fill', "#C92E27");
    }
    municipGroup.selectAll("path").attr("fill", null);
    for (entry in municipList) {
        municipGroup.selectAll("path").filter(function(d) {
            return d.properties.PCODE_PH2 === entry
        }).attr('fill', "#9A7F6A");
    }
    brgyGroup.selectAll("path").attr("fill", null);
    for (entry in brgyList) {
        brgyGroup.selectAll("path").filter(function(d) {
            return d.properties.PCODE_PH3 === entry
        }).attr('fill', "#ECE3B2");
    }
}
...
function zoomOut() {
    ...
    if (activeMunicip === true) {
        ...
        changePartnerFilter();
        svg.transition().duration(750).call(zoom.translate([0, 0]).scale(1)
            .event);
    }
}
        ...
        if (activeMunicip.length !== 0) {
            ...
            municipGroup.selectAll(".active").each(function(d) {
                ...
                var bounds = path.bounds(d),
                    ...
                    y = (bounds[0][1] + bounds[1][1]) / 2,
                    scale = 0.9 / Math.max(dx / width, dy / height),
                    translate = [width / 2 - scale * x, height / 2 -
                        scale * y
                    ];
                svg.transition().duration(750).call(zoom.translate(
                    translate).scale(scale).event);
            });
...
function showDisclaimer() {
    window.alert(
        "Program data is subject to change. Data may be incomplete. For official reports please refer to documents published by the Philippine Red Cross and/or IFRC. The maps on this page do not imply the expression of any opinion concerning the legal status of a territory or of its authorities."
    );
}
d3Start();